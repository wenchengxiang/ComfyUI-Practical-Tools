...
function addNodeToSelectedOrCursor(nodeType, side) {
	...
	if (selected.length > 0) {
		for (const n of selected) window.ptSetGet.addNode(nodeType, n, { side, offset: 30 });
	} else {
...
function findSetterByName(graph, name) {
	...
	for (const g of getGraphAncestors(graph)) {
		if (!g?._nodes) continue;
		for (const node of g._nodes) {
			if (node.type === 'PTSetNode' && node.widgets[0].value === name) {
				return { node, graph: g };
			}
		}
	}
...
function findGettersByName(graph, name) {
	...
	return collectNodesOfType(graphs, 'PTGetNode')
		.filter(entry => entry.node.widgets[0].value === name);
}

// Get all visible PTSetNode names for a PTGetNode's combo dropdown.
...
function getVisibleSetNames(graph, filterType) {
	...
	const entries = collectNodesOfType(ancestors, 'PTSetNode');
...
// Force every PTGetNode (root + subgraphs) to swap its widget.options reference so Vue re-reads values.
function refreshAllPTGetNodeCombos(graph) {
	const root = findRootGraph(graph);
	if (!root) return;
	const allGraphs = [root];
	const subgraphs = root._subgraphs || root.subgraphs;
	...
	for (const g of allGraphs) {
		if (!g?._nodes) continue;
		for (const node of g._nodes) {
			if (node.type === 'PTGetNode') node._refreshComboOptions?.();
		}
	}
}

// Exposed globally for use in contextmenu.js
window.ptSetGet = window.ptSetGet || {};
window.ptSetGet.convertOutputsToSetGet = convertOutputsToSetGet;
window.ptSetGet.snapshotSelectedNodes = snapshotSelectedNodes;
	...
	for (let slotIdx = 0; slotIdx < node.outputs.length; slotIdx++) {
		...
		for (const linkId of [...output.links]) {
			...
			if (link) {
				...
				if (targetNode && targetNode.type !== 'PTSetNode' && targetNode.type !== 'PTGetNode') {
		...
		// Create Set node
		const setNode = LiteGraph.createNode("PTSetNode");
		if (!setNode) continue;
		setNode.pos = [
		...
		for (const target of targets) {
			...
			const getNode = LiteGraph.createNode("PTGetNode");
			if (!getNode) continue;
			getNode.pos = [
...
function convertAllSetGetToLinks(graph) {
	...
	const allSetEntries = findAllNodesOfType(rootGraph, 'PTSetNode');
	...
	for (const { node: setNode, graph: g } of [...findAllNodesOfType(rootGraph, 'PTSetNode')]) {
...
function convertCrossGraphSetGet(setNode, setGraph, crossGraphGetters) {
	// Find what's connected to the PTSetNode's input
		...
		if (sgNodeForGetter && setGraph === rootGraph) {
			...
			for (const conn of connections) {
				if (conn.targetId === subgraph.outputNode?.id) {
					// PTGetNode fed a SubgraphOutput — bypass it and connect source directly in parent graph.
			...
			console.warn(`[Practical-Tools]: Cannot convert cross-graph Set/Get "${setNode.widgets[0].value}" between sibling subgraphs`);
		}
	}

	// Remove PTSetNode if all its getters have been handled (check if any same-graph getters remain)
	const remainingGetters = setGraph._nodes.filter(
		n => n.type === 'PTGetNode' && n.widgets[0].value === setNode.widgets[0].value
	);
...
function convertSetGetToLinks(graph, setNode) {
	...
	// Collect all consumer connections from Get nodes and PTSetNode's own output passthrough
	const connections = [];
...
app.registerExtension({
	name: "PracticalTools.SetNode",
	registerCustomNodes() {
		class PTSetNode extends LGraphNode {
			static title = "Set";
			static category = "Practical-Tools/SetGet";
			...
			constructor(title) {
				...
				this.properties["Node name for S&R"] = "PTSetNode";
				this.properties["aux_id"] = "wenchengxiang/ComfyUI-Practical-Tools";
			...
			validateName(graph, sameGraphOnly) {
				...
				if (widgetValue !== '') {
					...
					const scopedPTSetNodes = collectNodesOfType(scopeGraphs, 'PTSetNode');
					scopedPTSetNodes.forEach(entry => {
						if (entry.node !== this) {
			...
			onAdded() {
				...
				// Vue mode doesn't always re-extract PTGetNode options on PTSetNode add; skip during workflow load.
				if (LiteGraph.vueNodesMode && this.graph && !app.configuringGraph) {
					refreshAllPTGetNodeCombos(this.graph);
				}
			}
			...
			onRemoved() {
				...
				const g = this.graph;
				if (!g) return;
				// Defer: onRemoved fires before _nodes is spliced, so getters still see this PTSetNode.
				setTimeout(() => refreshAllPTGetNodeCombos(g), 0);
			}
			...
			getExtraMenuOptions(_, options) {
					...
					},
					{
						...
						callback: () => {
							for (const n of this.graph._nodes) {
								if (n.type === "PTGetNode" || n.type === "PTSetNode") n.drawConnection = false;
							}
							this.canvas.setDirty(true, true);
						},
				...
				options.unshift({
					...
					callback: () => {
						...
						for (const n of snapshotSelectedNodes(this, 'PTSetNode')) convertSetGetToLinks(graph, n);
					},
				});
				options.unshift({
					content: "Add paired PTGetNode",
					callback: () => {
						const graph = this.graph;
						const getNode = LiteGraph.createNode("PTGetNode");
						if (!getNode) return;
		...
		LiteGraph.registerNodeType("PTSetNode", PTSetNode);
		PTSetNode.category = "Practical-Tools/SetGet";
	},
});

// region PTGetNode
app.registerExtension({
	name: "PracticalTools.GetNode",
	registerCustomNodes() {
		class PTGetNode extends LGraphNode {
			static title = "Get";
			static category = "Practical-Tools/SetGet";
			serialize_widgets = true;
			drawConnection = false;
			...
			constructor(title) {
				...
				this.properties["Node name for S&R"] = "PTGetNode";
				this.properties["aux_id"] = "wenchengxiang/ComfyUI-Practical-Tools";
				this.isVirtualNode = true; // This node is purely frontend and does not impact the resulting prompt so should not be serialized
				const comboOptions = {
				...
				constantWidget.onClick = (params) => {
					...
					// Color each entry's left border by its PTSetNode's type.
			...
			onConfigure() {
				if (this._justAdded && !app.configuringGraph) {
					const name = this.widgets[0].value;
					if (name) {
						// Check if our paired PTSetNode was renamed during this paste
			...
			getInputLink(slot) {
				...
				const setter = this.graph?._nodes?.find(
					n => n.type === 'PTSetNode' && n.widgets[0].value === name
				);
				if (setter) {
				...
				if (name && !findSetterByName(this.graph, name)) {
					showAlert("No PTSetNode found for " + name + "(" + this.type + ")", this);
				}
				return null;
			}
			...
			resolveVirtualOutput(slot) {
				...
				// Warn if multiple PTSetNodes with this name exist in scope
				const scopeGraphs = getGraphAncestors(this.graph);
				const scopedSetters = collectNodesOfType(scopeGraphs, 'PTSetNode')
					.filter(e => e.node.widgets[0].value === name);
				if (scopedSetters.length > 1) {
					showAlert(`Multiple PTSetNodes named "${name}" found in scope. Rename duplicates or use "Convert to links" to resolve`, [this, ...scopedSetters.map(e => e.node)]);
					return undefined;
				}
					...
					options.unshift(
						{
							...
							callback: () => {
								const graph = this.graph;
								const setters = new Set(snapshotSelectedNodes(this, 'PTGetNode')
						...
						},
						{
							...
							callback: () => {
								...
								// Toggle on the PTSetNode so the canvas-level hook draws it
		...
		LiteGraph.registerNodeType("PTGetNode", PTGetNode);
		PTGetNode.category = "Practical-Tools/SetGet";
	},
});

// region UI: commands, keybindings, canvas menu, draw hook, settings
app.registerExtension({
	name: "PracticalTools.SetGetUI",
	settings: [],
	commands: [
		{
			id: "PracticalTools.AddPTSetNodeToSelected",
			label: "Add Set node to selected / at cursor",
			function: () => addNodeToSelectedOrCursor("PTSetNode", "right"),
		},
		{
			id: "PracticalTools.AddPTGetNodeAtCursor",
			label: "Add Get node to selected / at cursor",
			function: () => addNodeToSelectedOrCursor("PTGetNode", "left"),
		},

	],
	getCanvasMenuItems() { return []; },
	setup() {
		// Double-click PTGetNode to jump to its PTSetNode (works in both legacy and Vue modes)
		...
		LGraphCanvas.prototype.computeVisibleNodes = function () {
			...
			for (const node of this.graph._nodes) {
				if ((node.type === "PTSetNode" || node.type === "PTGetNode") && node.drawConnection && !visible.includes(node)) {
					visible.push(node);
				}
			}
			return visible;
		...
		app.canvas.onDrawBackground = function(ctx, visibleArea) {
			...
			if (mode === "selected") {
				...
				if (!sel.length) {
					...
					if (!graph._nodes.some(n => n.type === 'PTSetNode' && n.drawConnection)) return;
				} else {
					selectedNames = new Set();
					for (const n of sel) {
						if (n.type === 'PTSetNode' || n.type === 'PTGetNode') {
							selectedNames.add(n.widgets?.[0]?.value ?? n.title);
						}
					}
					if (!selectedNames.size && !graph._nodes.some(n => n.type === 'PTSetNode' && n.drawConnection)) return;
				}
			} else if (mode === "never") {
				if (!graph._nodes.some(n => n.type === 'PTSetNode' && n.drawConnection)) return;
			}

			// Collect all PTGetNodes once across current graph + descendants, grouped by name.
			// This avoids re-discovering descendant graphs and re-scanning nodes per PTSetNode.
			const gettersByName = new Map();
			const scopeGraphs = [graph, ...getGraphDescendants(graph)];
			for (const g of scopeGraphs) {
				if (!g?._nodes) continue;
				for (const node of g._nodes) {
					if (node.type !== 'PTGetNode') continue;
			...
			for (const setNode of graph._nodes) {
				if (setNode.type !== 'PTSetNode') continue;
		...
		LGraphCanvas.prototype.createDefaultNodeForSlot = function(optPass) {
			...
			const newNode = LiteGraph.createNode(isFrom ? "PTSetNode" : "PTGetNode");
...
app.registerExtension({
	name: "PracticalTools.CrossGraphSetGet",
	setup() {
		let patched = false;
				...
				try {
					...
					if (subgraphNode) {
						...
						if (dtos.length > 0) {
							...
							console.log(`[Practical-Tools] Cross-graph Set/Get: frontend native support ${hasNativeSupport ? 'detected, skipping patch' : 'not found, applying patch'}`);
							if (!hasNativeSupport) {
								...
								proto.resolveOutput = function(slot, type, visited) {
									if (typeof this.node?.resolveVirtualOutput === 'function') {
										...
										if (virtualSource) {
											...
											throw new Error(`[Practical-Tools]: No DTO found for cross-graph source node [${virtualSource.node.id}]`);
										}
									}
					...
					console.warn('[Practical-Tools] Failed to probe ExecutableNodeDTO for cross-graph patch:', e);
				}
			}
...
app.registerExtension({
	name: "PracticalTools.LinkToSetGet",
	setup() {
