...
分类：Practical-Tools/FlashVSR
已去除自动下载功能，模型文件需手动放置到 models/FlashVSR/ 目录。
"""
import os
import sys
import gc
...
from flashvsr.models.utils import clean_vram, get_device_list, Buffer_LQ4x_Proj, Causal_LQ4x_Proj
from flashvsr.models import wan_video_dit

device_choices = get_device_list()

# posi_prompt.pth 路径（在 flashvsr 包目录内）
...
def init_pipeline(model, mode, device, dtype, alt_vae="none"):
    ...
    pipe.init_cross_kv(prompt_path=_PROMPT_PATH)
...
class FlashVSR:
    """FlashVSR Ultra-Fast 单节点版本"""

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "frames": ("IMAGE", {"tooltip": "Sequential video frames as IMAGE tensor batch"}),
                "model": (["FlashVSR", "FlashVSR-v1.1"], {"default": "FlashVSR-v1.1", "tooltip": "Model version."}),
                "mode": (["tiny", "tiny-long", "full"], {"default": "tiny", "tooltip": 'Using "tiny-long" mode can significantly reduce VRAM used with long video input.'}),
                "scale": ("INT", {"default": 2, "min": 1, "max": sys.maxsize}),
                "color_fix": ("BOOLEAN", {"default": True, "tooltip": "Use color fix algorithm to preserve original colors."}),
                "tiled_vae": ("BOOLEAN", {"default": True, "tooltip": "Disable tiling: faster decode but higher VRAM usage."}),
                "tiled_dit": ("BOOLEAN", {"default": True, "tooltip": "Significantly reduces VRAM usage at the cost of speed."}),
                "unload_dit": ("BOOLEAN", {"default": False, "tooltip": "Unload DiT before decoding to reduce VRAM peak at the cost of speed."}),
                "tile_size": ("INT", {"default": 256, "min": 32, "max": 1024, "step": 32, "tooltip": "DiT tile size; 0 disables tiling."}),
                "tile_overlap": ("INT", {"default": 24, "min": 8, "max": 512, "step": 8}),
    ...
    def main(self, model, frames, mode, scale, color_fix, tiled_vae, tiled_dit, unload_dit, tile_size, tile_overlap, seed):
        ...
        output = flashvsr(pipe, frames, scale, color_fix, tiled_vae, tiled_dit, tile_size, tile_overlap, unload_dit, 2.0, 3.0, 11, seed, True)
        return (output,)
